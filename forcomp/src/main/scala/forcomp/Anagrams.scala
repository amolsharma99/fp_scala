...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.groupBy(ch => ch.toLower).mapValues(_.size).toList.sorted
  }
   ...
   */
   def subtract(x: Occurrences, y: Occurrences): Occurrences = y.foldLeft(x.toMap)(subtract).toList.filter(_._2 > 0).sorted
   
   def subtract(x: Map[Char, Int], term: (Char, Int)): Map[Char, Int] = {
    val (char, freq) = term
    x + (char -> (x.get(char).get - freq))
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsInternal(occurrences: Occurrences): List[Sentence] = {
      occurrences match {
        case Nil => List(List())
        case _ => {
          for {
          ...
          } yield wordInDictionaryForCombination :: remainingWords
        }
      }
    }
