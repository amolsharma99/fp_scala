...
object Huffman {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeInner(currentTree: CodeTree, remainingBits: List[Bit]): (Char, List[Bit]) = {
      currentTree match {
        case Leaf(char, weight) => (char, remainingBits)
        case Fork(left, right, ch, wt) => remainingBits match {
          case x :: xs if (x == 0) => decodeInner(left, xs)
          case x :: xs if (x == 1) => decodeInner(right, xs)
          case _ => throw new IllegalArgumentException("Invalid input. Cannot decode!")
        }
      }
    }
    val (char, remainingBits) = decodeInner(tree, bits)
    remainingBits.isEmpty match {
      case true => List[Char](char)
      case false => char :: decode(tree, remainingBits)
    }
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeCharacter(currentTree: CodeTree, charToEncode: Char): List[Bit] = {
      currentTree match {
        case Leaf(char, weight) if (char != charToEncode) => throw new IllegalArgumentException("Invalid input. Cannot encode!")
        case Leaf(char, weight) if (char == charToEncode) => Nil
        case Fork(left, right, ch, wt) if (chars(left).contains(charToEncode)) => 0 :: encodeCharacter(left, charToEncode)
        case Fork(left, right, ch, wt) if (chars(right).contains(charToEncode)) => 1 :: encodeCharacter(right, charToEncode)
      }
    }
